...
class GenericErrorHandler {
  ...
  static GenericErrorHandler newInstance() => _instance = GenericErrorHandler._();
  ...
  Future<void> handle<T extends Bloc>({
    ...
    assert((onRetry == null) ^ (event == null), 'Must provide either onRetry or an event');
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) async {
        final retry = await showDialog(
              context: context,
              barrierDismissible: failure is SessionEndedFailure ? false : dismissible,
              builder: (context) => NoConnection(dismissible: dismissible, retriable: retriable, failure: failure),
            ) ??
            false;
        isShowing = false;
        if (retry) {
